...
def obtain_data(x_available, data_type, path_to_literature_data, Re_available):
    all_x_data = []
    for x_value in x_available:
        data = []

        # Read data and append to list
        for Re_number in Re_available:
            Rapp2009_csv = path_to_literature_data + '/'+ Re_number + '/'+ '_Rapp2009' + str(data_type) + '_x_' + str(x_value) + '.csv'
...
def plot_onto_geometry(x_available, Re, all_x_data, folder_to_save, x_vector, y_bottom, y_top, scale_factor,
    ...
    for i, x_value in enumerate(x_available):
        ...
        for j, dataset in enumerate(data_x):
            ...
            elif j == 1:      # Data is from Rapp 10600
                # Scale data to plot and remove first column
                dataset[0,:] = (scale_factor * dataset[0,:]) + x_value
                dataset = numpy.delete(dataset, 0, 1)
    ...
    if show_title is True:
        ...
        ax.set_title(title + ": experimental Data (Rapp 2009)")
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(folder_to_save + "experimental_data_in_geometry_" + str(data_type) + ".png", dpi=800, bbox_inches='tight',pad_inches = 0)
    # plt.close(fig)
    ax.clear()
