# Name   : plot_data_time_stepping.py
...
# labels = ["Lethe baseline - 600s", "Lethe baseline - 700s", "Lethe baseline - 800s", "Lethe baseline - 900s", "Lethe baseline - 1000s", "Lethe baseline - 1100s", "Lethe baseline - 1200s"]
labels = ["Lethe - t=0.1s", "Lethe - t=0.05s", "Lethe - t=0.025s", "Lethe - t=0.0125s"]
...
def plot_to_png(Breuer2009_data, Rapp2009_data, lethe_data, data_type, x_value, labels,
    ...
    if zoom_in_plots is True:
        ...
        ax2 = fig.add_subplot(2,7,(6,7), zorder = 1)
        ax3 = fig.add_subplot(2,7,(13,14), zorder = 1)  
    else:
        fig, ax = plt.subplots()
        ...
        if file_name is not None:
            if lethe_data != []:
                ...
                if zoom_in_plots is True:
                    ...
                    ax3.plot(lethe_data[chunk], lethe_data[chunk+1], '--', color=colors[index], linewidth = 1.2, zorder = 3, mfc = 'none') 
    ...
    # Plot Breuer data
    if Breuer2009_data != []:
        ax.plot(Breuer2009_data[0], Breuer2009_data[1], ':',  alpha=1, color="k", linewidth = 1.2,
                label='LESOCC - Breuer 2009', zorder = 2)
        if zoom_in_plots is True:
            ax2.plot(Breuer2009_data[0], Breuer2009_data[1], ':',  alpha=1, color="k", linewidth = 1.2,
                    zorder = 2)
            ax3.plot(Breuer2009_data[0], Breuer2009_data[1], ':',  alpha=1, color="k", linewidth = 1.2,
                    zorder = 2)
    ...
    ax.xaxis.set_major_locator(ticker.MultipleLocator(0.5)) #0.5 for velocity 0.1 for reynolds
    ...
    if zoom_in_plots is True:
        ...
        x1 = 0.9; x2 = 1.2; y1 = 2.7; y2 = 3.0 #for average velocity x = 0.5
        # x1 = 0.9; x2 = 1.2; y1 = 2.7; y2 = 3.0 #for average velocity x = 2

        # x1 = 0; x2 = 0.04; y1 = 2.7; y2 = 3.1 #for reynolds shear stress uv at x=0.5
        # x1 = 0; x2 = 0.04; y1 = 2.6; y2 = 3.2 #for reynolds shear stress uv at x=2
        ax2.set_xbound(x1, x2)
        ax2.set_ybound(y1, y2)
        ax2.xaxis.set_major_locator(ticker.MultipleLocator(0.1)) #0.1 for velocity 0.02 for reynolds
        ax2.yaxis.set_major_locator(ticker.MultipleLocator(0.1)) #0.1 for average velocity 0.2 for reynolds
        ...
        ax2_1 = ConnectionPatch(xyA=(x2, y2 - (y2-y1)/2), coordsA=ax.transData, xyB=(x1, y1 + (y2-y1)/2), coordsB=ax2.transData, color = 'gray',linewidth=0.5, arrowstyle ="->", zorder = 0)
        fig.add_artist(ax2_1)

        #2nd. Zoom in bottom right
        x1 = -0.1; x2 = 0.1; y1 = 0.8; y2 = 1.2 #for average velocity x = 0.5
        # x1 = -0.3; x2 = 0.1; y1 = -0.1; y2 = 0.3 #for average velocity x = 2

        # x1 = 0; x2 = 0.02; y1 = 0.8; y2 = 1.0 #for reynolds normal stress at x=0.5
        # x1 = 0; x2 = 0.04; y1 = 0; y2 = 0.4 #for reynolds shear stress uv at x=2

        ax3.set_xbound(x1, x2)
        ax3.set_ybound(y1, y2)
        ax3.xaxis.set_major_locator(ticker.MultipleLocator(0.1)) #0.02 for Reynolds stress at x=2 at x=0.5 0.02     0.1 for velocity
        ax3.yaxis.set_major_locator(ticker.MultipleLocator(0.1))#0.2 for Reynolds stress at x=2 at x=0.5 0.1    0.1 for velocity

        ax.add_patch(patches.Rectangle((x1,y1),(x2-x1),(y2-y1),linewidth=0.5, edgecolor='gray', facecolor = 'none'))
        ax3_1 = ConnectionPatch(xyA=(x2, y1 + (y2-y1)/2), coordsA=ax.transData, xyB=(x1, y1 + (y2-y1)/2), coordsB=ax3.transData, color = 'gray',linewidth=0.5, arrowstyle ="->", zorder = 0)
    ...
    if zoom_in_plots is True:
        fig.savefig(
            folder_to_save_png + "graph_" + data_type + "_x_" + str(x_value) + "_with_zoom_in_timestep.png",
            dpi=600)
    else:
        fig.savefig(
